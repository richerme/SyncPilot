...
Los skills viven en `.codex/skills/` (stubs) y referencian la implementacion completa en `.claude/skills/`.
...
Configurados en `.codex/mcp.json`.
...
```
.codex/                        # Configuracion Codex CLI
├── skills/                    # Skills (stubs → .claude/skills/)
│   ├── new-app/
│   ├── add-login/
│   ├── add-payments/
│   ├── add-emails/
...
│   ├── coolify-deployer/
│   └── video-visuals/
├── mcp.json                   # MCPs configurados (con tokens reales)
└── example.mcp.json           # MCPs ejemplo (sin secrets)

.claude/                       # Configuracion Claude Code
├── skills/                    # Skills completos (implementacion real)
