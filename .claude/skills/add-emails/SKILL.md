...
name: add-emails
description: |
  Integra sistema de correos transaccionales con Resend + React Email en tu proyecto Next.js + Prisma.
...
```prisma
// Add to User model (already exists from add-login):
//   emailNotificationsEnabled Boolean @default(true)
```

Then run: `npx prisma migrate dev --name add_email_preferences`
...
1. Verificar que `/add-login` fue ejecutado (buscar `src/lib/auth.ts`).
2. Instalar: `npm install resend @react-email/components`
3. Actualizar Prisma schema con campo `emailNotificationsEnabled` en User.
4. Crear TODOS los archivos listados.
5. Aplicar migracion: `npx prisma migrate dev --name add_email_preferences`
...
Archivos creados:
  prisma/schema.prisma (emailNotificationsEnabled field)
