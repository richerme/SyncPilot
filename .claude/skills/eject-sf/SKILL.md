...
Aplicacion web construida con Next.js 16 + PostgreSQL + Prisma.
...
└── lib/          # Configuraciones (DB, Auth, etc.)
\`\`\`
...
Listo para deploy en Coolify (VPS):

\`\`\`bash
# Crear repo privado y push
gh repo create mi-app --private --source=. --push

# Configurar en Coolify:
# 1. Crear app con el repo
# 2. Crear PostgreSQL
# 3. Configurar env vars (DATABASE_URL, AUTH_SECRET, etc.)
# 4. Deploy
\`\`\`
```
